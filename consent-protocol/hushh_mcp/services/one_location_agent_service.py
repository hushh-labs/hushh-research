...
class OneLocationAgentService:
    ...
    def _expire_stale_grants(self, user_id: str | None) -> None:
        ...
        expired = self._execute_many(
            ...
            UPDATE one_location_share_grants AS location_grant
            SET status = 'expired', updated_at = NOW()
            FROM stale
            WHERE location_grant.id = stale.id
            RETURNING
              location_grant.id,
              location_grant.owner_user_id,
              location_grant.recipient_user_id,
              location_grant.expires_at
            """,
            {"user_id": user_id},
        )
        for row in expired:
