...
- For update_pkm, infer the target `domain` from the PKM context provided for routing (the user's existing PKM domain keys are listed there). Set `field_path` to the attribute being changed (dot notation if nested, e.g. "address" or "address.line1") and `proposed_value` to the new value. Include `current_value` only if the existing value is visible in the PKM context.
                ...
                parameters=_schema_object(
                    {
                        "domain": _schema_string(
                            "Target PKM domain key to update, inferred from the PKM context "
                            "provided for routing (for example identity, finance_portfolio, "
                            "financial_domain)."
                        ),
