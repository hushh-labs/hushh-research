"""Freshest published browser context for an in-flight agent task.

This module owns session-local state for typed agent actions. Legacy Live-named
imports remain available through a compatibility wrapper; no Live transport calls it. A task's
initial snapshot can become stale after navigation, while browser context
updates are delivered separately from the invocation that is already running.

The observable failure: after a navigation the browser has the new screen while
the goal tools keep reporting the prior screen. A cross-screen journey could
therefore never continue, and each retry re-read the same frozen value instead
of converging on the truth.
...
It lives in ``hushh_mcp.services`` because the dependency runs one way:
``api`` may import ``hushh_mcp``, never the reverse. The browser-context
publisher (writer) and action tools (reader) can reach it here.

Scale plane (AGENTS.md, "Postgres now, Redis later"): process-local is correct
today because an in-flight task is pinned to one instance, so there is no
cross-instance reader. If task state later migrates between instances, swap the
dict for the shared session tier behind these same three functions and no caller
changes.
"""
...
_TASK_CONTEXT_BY_SESSION: dict[str, dict[str, Any]] = {}


def publish_agent_task_context(session_id: str | None, context: dict[str, Any]) -> None:
    """Record the newest sanitized context for a task."""
    clean_id = str(session_id or "").strip()
    if clean_id and isinstance(context, dict):
        _TASK_CONTEXT_BY_SESSION[clean_id] = context


def read_agent_task_context(session_id: str | None) -> dict[str, Any] | None:
    """Return the newest sanitized context for a task, if published."""
    clean_id = str(session_id or "").strip()
    if not clean_id:
        return None
    return _TASK_CONTEXT_BY_SESSION.get(clean_id)


def clear_agent_task_context(session_id: str | None) -> None:
    """Drop a task's context when it closes."""
    clean_id = str(session_id or "").strip()
    if clean_id:
        _TASK_CONTEXT_BY_SESSION.pop(clean_id, None)
...
# Stable aliases for callers that have not yet moved to the command-neutral
# task-context API. Active runtime imports use the names above.
