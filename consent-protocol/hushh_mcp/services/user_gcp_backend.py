...
class UserGcpBackend:
    ...
    def live(self) -> bool:
        ...
        state. Plan mode (no ``HUSSH_USER_GCP_LIVE``) returns a rendered handle with
    ...
    async def erase_signing_secret(
        self,
        *,
        action: dict[str, Any],
        state: dict[str, Any],
        ...
        """Reconcile one captured signing secret using the owner's bootstrap authority."""
        import asyncio
        ...
        if not self._live or not self._user_project or not self._bootstrap_sa:
            raise RuntimeError("secret erasure authority unavailable")
        token = await asyncio.to_thread(
            mint_bootstrap_token, bootstrap_sa=self._bootstrap_sa.removeprefix("serviceAccount:")
        )
        deleter = build_gcp_deleter(
            ...
            retain_secret_receipt=retain_receipt,
        )
        await deleter(action)

    async def erase_runtime_account(
        self,
        *,
        action: dict[str, Any],
        state: dict[str, Any],
        ...
        """Reconcile one captured disabled runtime service account using the owner's bootstrap authority."""
        import asyncio
        ...
        if not self._live or not self._user_project or not self._bootstrap_sa:
            raise RuntimeError("runtime account erasure authority unavailable")
        token = await asyncio.to_thread(
            mint_bootstrap_token, bootstrap_sa=self._bootstrap_sa.removeprefix("serviceAccount:")
        )
        deleter = build_gcp_deleter(
            ...
            retain_account_receipt=retain_receipt,
        )
        await deleter(action)
    ...
    async def erase_repository_grant(
        self,
        *,
        evidence: dict[str, Any],
        state: dict[str, Any],
        ...
        """Reconcile the recorded repository writer grant; preserve shared storage."""
        import asyncio
        ...
        if not self._live or not self._user_project or not self._bootstrap_sa:
            raise RuntimeError("runtime grant erasure authority unavailable")
        binding = evidence["bindingObservation"]
        token = await asyncio.to_thread(
            mint_bootstrap_token, bootstrap_sa=self._bootstrap_sa.removeprefix("serviceAccount:")
        )
        deleter = build_gcp_deleter(
