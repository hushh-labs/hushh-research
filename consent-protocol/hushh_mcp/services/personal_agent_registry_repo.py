...
class PersonalAgentRegistryRepo:
    ...
    async def verify_erasure_files_preflight(self, *, user_id: str, reservation: dict) -> bool:
        response = await asyncio.to_thread(
            self._db().execute_raw,
            "SELECT public.verify_erasure_files_preflight(:owner, :attempt, "
            "CAST(:expected AS jsonb)) AS verified",
            {
                "owner": user_id,
                "attempt": reservation["attemptId"],
    ...
    async def retain_erasure_files_receipt(
        self, *, user_id: str, reservation: dict, kind: str, stage: str, receipt: dict
    ) -> bool:
        """Retain Files resource removal under the existing owner erasure reservation."""
        response = await asyncio.to_thread(
            self._db().execute_raw,
            "SELECT public.retain_erasure_files_receipt(:owner, :attempt, "
            "CAST(:expected AS jsonb), :kind, :stage, CAST(:receipt AS jsonb)) AS retained",
            {
                "owner": user_id,
                "attempt": reservation["attemptId"],
    ...
    async def record_binding(
        ...
        *,
        ...
        return await direct_admission.record_binding(
            self._db(),
            user_id=user_id,
            device_id=device_id,
            record=record,
    ...
    async def record_direct_readiness(
        ...
        *,
        ...
        return await direct_admission.record_direct_readiness(
            self._db(),
            user_id=user_id,
            hushh_id=hushh_id,
    ...
    async def record_direct_ingress_observed(
        ...
        *,
        ...
        return await direct_admission.record_direct_ingress_observed(
            self._db(),
            user_id=user_id,
            hushh_id=hushh_id,
            service_uid=service_uid,
