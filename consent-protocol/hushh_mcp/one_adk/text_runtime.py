...
async def _bounded_adk_events(
    ...
    *,
    ...
    """Bound a single ADK iterator without moving its context across tasks."""
    events = bounded_adk_events(
        source,
        first_event_timeout_s=(
            _FIRST_EVENT_TIMEOUT_SECONDS if first_event_timeout is None else first_event_timeout
        ),
        between_event_timeout_s=(
            _BETWEEN_EVENT_TIMEOUT_SECONDS
            if between_event_timeout is None
            else between_event_timeout
        ),
        total_timeout_s=_TOTAL_TURN_TIMEOUT_SECONDS if total_timeout is None else total_timeout,
    )
    try:
...
async def _stream_one_text_turn_once(
    *,
    ...
    async for event in _bounded_adk_events(
        ...
        # Partials suppress only their own model step's aggregate, not a later
        # answer after a tool call.
        saw_partial_text = False
        ...
        try:
            async for event in _stream_one_text_turn_once(
                ...
                if event.kind == "source":
                    # Provenance remains replay-safe before a visible answer.
                    yield event
                    continue
                replay_boundary_crossed = True
                yield event
