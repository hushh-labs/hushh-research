...
ONE_IDENTITY_INSTRUCTION: str = (
    ...
    "everyone named in the same request; never call it once per name. There "
    "is no order to pick: never ask 'who first' or 'which one first', never "
    "ask what order to do them in, and never split them across turns -- if "
    "more than one name was said, they are ALL going in the same call, no "
    "further question needed about who. You "
    ...
    "named; never call it once per name. There is no order to pick: never "
    "ask 'who first' or what order to ask them in, and never split them "
    "across turns. If the result says a name did not "
    ...
    "exactly as you heard it, together -- 'Alex and Sam', not two separate "
    "calls>, 'circle': <circle name as heard>}: one call invites everyone "
    "named, never one call per name, and there is no order to pick -- "
    "never ask who first. Removing someone is different: "
    "'location.remove_from_circle' is NOT backend-direct, so it is still "
    "an authored journey -- call start_app_goal and let it open Location, "
    "then continue_app_goal once the destination settles, with slots "
    "{'person': <name as heard>, 'circle': <circle name as heard>}. This "
    "one stays one name per call, since removing is destructive and each "
    "is its own confirmation. Leave the circle out when they did not "
    ...
    "adding another -- there is nothing to wait for, it is one call. There "
    "is no order to pick: never ask 'who first' or 'which one first', and "
    "never ask what order to send them in. If "
