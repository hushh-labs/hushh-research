"""Browser-frame trust boundary for Agent One command and chat context.

Every browser context payload crosses this module
before it can influence session state, the system instruction, or tool
authority. Pure functions only: no socket, no session access, and no side
effects beyond bounded INFO logs.
...
AGENT_CONTEXT_ARRAY_CAP = AVAILABLE_ACTION_IDS_CAP
...
AGENT_SCREEN_STATE_KEY_CAP = 48
AGENT_CAPABILITY_CAP = 10
...
def bounded_text(value: Any, limit: int = AGENT_CONTEXT_STRING_CAP) -> str:
...
def sanitize_agent_context(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # canonical action snapshot is nested under ``one_voice_context`` (a stable
    # wire key). Legacy streaming payloads flatten that snapshot before
    # transport. Normalize both shapes so chat cannot fall into a
    # compatibility-permissive context merely because it used the shared
    # builder directly.
    ...
    submitted_raw = bounded_text_list(payload.get("available_action_ids"), AGENT_CONTEXT_ARRAY_CAP)
    ...
    if submitted_raw and not route_action_ids:
        ...
        # a missing index entry is diagnosable instead of a silent rejected command.
        logger.info(
            "one_agent_context_route_without_actions route=%s submitted=%d accepted=%d",
    ...
    return {
        ...
        "visible_modules": bounded_text_list(payload.get("visible_modules"), AGENT_MODULE_CAP),
        "visible_control_ids": bounded_text_list(
            payload.get("visible_control_ids"), AGENT_MODULE_CAP
        ),
        ...
        "portfolio_ready": payload.get("portfolio_ready") is True,
        "busy_operations": bounded_text_list(payload.get("busy_operations"), AGENT_MODULE_CAP),
...
def sanitize_dead_end(value: Any, submitted_action_ids: list[str]) -> dict[str, Any] | None:
    ...
    if remedy_action_id not in submitted_action_ids and not is_navigation_action(
        ...
        logger.info(
            "one_agent_context_dead_end_remedy_rejected remedy=%s",
            remedy_action_id,
        )
        return None
...
def sanitize_interaction_layer(
    ...
    visible_action_ids = [
        action_id
        for action_id in bounded_text_list(value.get("visible_action_ids"), AGENT_CONTEXT_ARRAY_CAP)
    ...
    return {
        ...
        "visible_control_ids": bounded_text_list(
            value.get("visible_control_ids"), AGENT_MODULE_CAP
        ),
        "options": options,
...
def sanitize_screen_state(value: Any) -> dict[str, Any]:
    ...
    for raw_key, raw_value in payload.items():
        if len(out) >= AGENT_SCREEN_STATE_CAP:
            break
        if not isinstance(raw_key, str):
            continue
        key = raw_key.strip()[:AGENT_SCREEN_STATE_KEY_CAP]
    ...
    return {
        ...
        "setup_capability_ids": [
            ...
            for capability in bounded_text_list(
                payload.get("setup_capability_ids"), AGENT_CAPABILITY_CAP
            )
...
def sanitize_voice_settings(value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "disabled_domains": [
            domain
            for domain in bounded_text_list(payload.get("disabled_domains"), AGENT_MODULE_CAP)
