...
_USER_AGENT = "hushh-research-sage/1.0 (personal research assistant)"
...
def _get_genai_client(model: str = _MODEL):
    ...
    key locally) -- same resolution as `pkm_highlight.py`'s Sage endpoints
    and `hushh_mcp/operons/kai/llm.py`. Reading GOOGLE_API_KEY directly
    bypasses Vertex ADC entirely, which is the auth mode this app actually
    runs under outside local dev."""
    try:
        ...
        logger.warning("[sage_citations] genai client unavailable: %s", exc)
        return None
