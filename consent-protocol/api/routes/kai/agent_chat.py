...
from hushh_mcp.services.agent_chat_turn_service import (
    AgentChatTurnEvent,
    AgentChatTurnInput,
    prepare_agent_chat_turn_stream,
...
def _stream_event(event: AgentChatTurnEvent) -> str:
    return _event(event.name, event.data)
...
async def stream_agent_chat(
    ...
    try:
        prepared = await prepare_agent_chat_turn_stream(
            AgentChatTurnInput(
                ...
                delegate_result=(
                    body.delegate_result.model_dump(by_alias=True, exclude_none=True)
            ...
            delegate_target_resolver=resolve_delegate_target,
            wired_specialist_checker=is_wired_specialist,
        )
    ...
    if prepared.conversation_id:
        headers["X-Agent-Conversation-Id"] = prepared.conversation_id
    if prepared.model:
        headers["X-Agent-Model"] = prepared.model
