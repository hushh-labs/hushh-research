...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM public.personal_agent_registry WHERE backend_metadata->'erasure' ? 'repositoryInventory') THEN RAISE EXCEPTION 'repository inventory requires preservation'; END IF; END $$;
...
DROP FUNCTION public.retain_erasure_repository_inventory(text,text,jsonb,jsonb);
DROP FUNCTION public.valid_erasure_repository_inventory(jsonb,jsonb);
COMMIT;
