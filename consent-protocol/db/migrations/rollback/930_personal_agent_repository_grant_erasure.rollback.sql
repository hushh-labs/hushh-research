...
DO $$ BEGIN IF EXISTS (SELECT 1 FROM public.personal_agent_registry WHERE backend_metadata->'erasure' ? 'repositoryGrantErasure') THEN RAISE EXCEPTION 'repository grant receipts require preservation'; END IF; END $$;
...
DROP FUNCTION public.retain_erasure_repository_grant_receipt(text,text,jsonb,text,jsonb);
DROP FUNCTION public.valid_erasure_repository_grant_append(jsonb,jsonb);
DROP FUNCTION public.valid_erasure_repository_grant_receipt(jsonb,text,jsonb);
COMMIT;
