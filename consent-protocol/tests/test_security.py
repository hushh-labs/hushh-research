...
# ---------------------------------------------------------------------------
# Identity PII key masking (Wave 2 / D-10)
# ---------------------------------------------------------------------------


class TestSanitizePayloadIdentityPII:
    def test_full_name_masked(self):
        out = sanitize_payload({"full_name": "Jane Doe"})
        assert "Jane Doe" not in str(out)
        assert out["full_name"] != "Jane Doe"

    def test_name_key_masked(self):
        out = sanitize_payload({"name": "Jane Doe"})
        assert "Jane Doe" not in str(out)

    def test_first_and_last_name_masked(self):
        out = sanitize_payload({"first_name": "Jane", "last_name": "Doe"})
        assert "Jane" not in str(out)
        assert "Doe" not in str(out)

    def test_address_masked(self):
        out = sanitize_payload({"address": "500 Market St"})
        assert "500 Market St" not in str(out)

    def test_address_components_masked(self):
        out = sanitize_payload({"line1": "500 Market St", "line2": "Apt 4", "street": "Market St"})
        assert "500 Market St" not in str(out)
        assert "Apt 4" not in str(out)
        assert "Market St" not in str(out)

    def test_date_of_birth_masked(self):
        out = sanitize_payload({"date_of_birth": "1990-01-01"})
        assert "1990-01-01" not in str(out)

    def test_dob_masked(self):
        out = sanitize_payload({"dob": "1990-01-01"})
        assert "1990-01-01" not in str(out)

    def test_passport_masked(self):
        out = sanitize_payload({"passport_number": "X1234567"})
        assert "X1234567" not in str(out)

    def test_passport_key_masked(self):
        out = sanitize_payload({"passport": "X1234567"})
        assert "X1234567" not in str(out)

    def test_national_id_masked(self):
        out = sanitize_payload({"national_id": "ABC123"})
        assert "ABC123" not in str(out)

    def test_ssn_defensively_masked(self):
        # D-A: no SSN is ever stored, but if it ever appears it must be masked.
        out = sanitize_payload({"ssn": "123-45-6789", "social_security": "123456789"})
        assert "123-45-6789" not in str(out)
        assert "123456789" not in str(out)

    def test_non_pii_scalar_preserved(self):
        out = sanitize_payload({"amount": 42, "active": True, "note": None})
        assert out == {"amount": 42, "active": True, "note": None}

    def test_non_pii_string_value_preserved(self):
        out = sanitize_payload({"status": "active"})
        assert out["status"] == "active"

    def test_original_payload_not_mutated(self):
        original = {"full_name": "Jane Doe", "address": "500 Market St"}
        sanitize_payload(original)
        assert original["full_name"] == "Jane Doe"
        assert original["address"] == "500 Market St"

    def test_nested_identity_masked(self):
        out = sanitize_payload(
            {"identity": {"full_name": "Jane Doe", "address": {"line1": "500 Market St"}}}
        )
        assert "Jane Doe" not in str(out)
        assert "500 Market St" not in str(out)

    def test_masked_value_is_string(self):
        out = sanitize_payload({"full_name": "Jane Doe"})
        assert isinstance(out["full_name"], str)
        assert "*" in out["full_name"]
