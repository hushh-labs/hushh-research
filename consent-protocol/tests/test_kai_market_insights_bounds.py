...
def _build_app() -> FastAPI:
    ...
    def _mock_require_firebase_auth():
        return "user_123"

    app.dependency_overrides[require_vault_owner_token] = _mock_require_vault_owner_token
    app.dependency_overrides[require_firebase_auth] = _mock_require_firebase_auth
...
def test_market_news_rejects_oversized_cursor_query():
    client = TestClient(_build_app())
    response = client.get(
        "/market/news/user_123",
        params={"cursor": "x" * (market_insights.NEWS_FEED_CURSOR_MAX_LENGTH + 1)},
    )

    assert response.status_code == 422


def test_market_news_baseline_rejects_oversized_cursor_query():
    client = TestClient(_build_app())
    response = client.get(
        "/market/news/baseline/user_123",
        params={"cursor": "x" * (market_insights.NEWS_FEED_CURSOR_MAX_LENGTH + 1)},
    )
