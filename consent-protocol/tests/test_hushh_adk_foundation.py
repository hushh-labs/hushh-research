...
class TestHushhAdkFoundation(unittest.TestCase):
    ...
    @patch("hushh_mcp.hushh_adk.tools.validate_token_with_db")
    def test_tool_decorator_success(self, mock_validate):
        """Test successful tool execution with valid context/token.

        Sync tools now run the same DB-backed validation as async tools
        (driven to completion via a blocking helper).
        """
        print("\n🧪 Testing @hushh_tool success...")

        # Setup mock validation success (async callable returning the tuple)
        mock_validate.side_effect = AsyncMock(return_value=(True, "OK", self.mock_token_obj))
    ...
    def test_tool_scope_fail(self, mock_validate):
        ...
        # Setup mock validation FAILURE (async callable returning the tuple)
        mock_validate.side_effect = AsyncMock(return_value=(False, "Scope mismatch", None))
...
def test_sync_hushh_tool_still_works():
    ...
    They now run the same DB-backed validation as async tools (driven to
    completion via a blocking helper).
    """
    from hushh_mcp.consent.token import issue_token
    from hushh_mcp.constants import ConsentScope
...
def test_sync_hushh_tool_rejects_db_revoked_token():
    """
    Sync @hushh_tool must reject a token revoked in DB but not in local
    memory - cross-instance revocation now holds for sync tools too.
    """
    from hushh_mcp.consent.token import issue_token
    from hushh_mcp.constants import ConsentScope

    token_obj = issue_token("user_sync_test2", "agent_test", ConsentScope.VAULT_OWNER)
    ...
    with patch.dict(sys.modules, {"hushh_mcp.services.consent_db": fake_module}):
        with patch.dict(os.environ, {"TESTING": "false"}, clear=False):
            with HushhContext(user_id="user_sync_test2", consent_token=token_obj.token):
                with pytest.raises(PermissionError):
                    dummy_sync_tool()
