...
def test_private_registry_migration_and_rollback_preserve_curated_rows(lifecycle):
    migration = (MIGRATIONS / "245_private_mcp_registration.sql").read_text()
    ...
    with lifecycle.db.engine.connect() as connection:
        connection.exec_driver_sql(
            (MIGRATIONS / "rollback/245_private_mcp_registration.rollback.sql").read_text()
        )
    assert sql(
        lifecycle,
...
async def test_private_registration_is_idempotent_owner_bound_and_non_authorizing(lifecycle):
    with lifecycle.db.engine.connect() as connection:
        connection.exec_driver_sql((MIGRATIONS / "245_private_mcp_registration.sql").read_text())
