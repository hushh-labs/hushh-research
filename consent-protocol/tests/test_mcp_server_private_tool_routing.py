...
Regression test for a routing bug: canonical_tool_name() only resolves the 5
published core_consent names, so every other already-canonical private tool
name (kai_*, list_ria_*, ...) resolved to None in mcp_server.call_tool()'s
HANDLERS lookup and in developer_context.is_tool_allowed()'s entitlement
check -- meaning the already-shipped kai_voice/ria_read tools were listed by
tools/list but every actual tools/call against them was unreachable. No
existing test called mcp_server.call_tool() directly for a private tool
name; every other test either loads a handler module standalone (bypassing
mcp_server.py) or tests is_tool_allowed()/visible_tool_names_for_groups() in
isolation.

This file closes that gap: it drives the actual @server.call_tool()-
decorated function end-to-end for a kai_voice tool, under a real
DeveloperPrincipal context.
"""
