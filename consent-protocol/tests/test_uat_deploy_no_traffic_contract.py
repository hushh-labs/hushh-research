...
def test_uat_runtime_capacity_is_bounded_and_revision_safe() -> None:
    workflow = _read(".github/workflows/deploy-uat.yml")
    # The backend's gcloud call moved out of the YAML and into
    # scripts/deploy/backend-deploy.sh, so the capacity flags now live in the script
    # while the substitutions that feed them stay in the YAML. Read the whole surface.
    backend_build = backend_deploy_surface()
    frontend_build = _read("deploy/frontend.cloudbuild.yaml")
...
def test_uat_uses_the_rehearsed_vertex_live_fallback_when_developer_credits_are_depleted() -> None:
    # The env assembly moved into scripts/deploy/backend-deploy.sh (and `add_env`
    # was renamed `append_optional_env` there); the substitution default stays in
    # the YAML. Read the whole deploy surface so both halves are covered.
    backend_build = backend_deploy_surface()
    uat_workflow = _read(".github/workflows/deploy-uat.yml")
    production_workflow = _read(".github/workflows/deploy-production.yml")
