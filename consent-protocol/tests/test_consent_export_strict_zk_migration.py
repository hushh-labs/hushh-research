...
MIGRATION_PATH = REPO_ROOT / "db" / "migrations" / "061_consent_exports_export_key_guard.sql"
...
def test_consent_exports_export_key_guard_is_in_release_and_pkm_lanes() -> None:
    ...
    assert "061_consent_exports_export_key_guard.sql" in ordered
    assert ordered.index("061_consent_exports_export_key_guard.sql") > ordered.index(
        "035_strict_zero_knowledge_consent_exports.sql"
    )
    assert "061_consent_exports_export_key_guard.sql" in manifest["groups"]["pkm"]
...
def test_consent_evolution_explicit_mode_runs_export_key_guard() -> None:
    ...
    assert '"061_consent_exports_export_key_guard.sql"' in migrate_py
