...
def test_named_circle_migrations_are_registered_in_release_order() -> None:
    ...
    migrations = [
        "133_one_location_named_circles.sql",
        "134_one_location_circle_connection_origins.sql",
        "135_one_location_circle_member_invites.sql",
    ]

    assert manifest["ordered_migrations"][-3:] == migrations
    assert all(migration in manifest["groups"]["iam"] for migration in migrations)
    assert schema["expected_migration_version"] == 135
...
def test_named_circle_base_schema_is_hash_only_and_additive() -> None:
    migration = (MIGRATIONS_DIR / "133_one_location_named_circles.sql").read_text(encoding="utf-8")
    schema = json.loads(SCHEMA_CONTRACT_PATH.read_text(encoding="utf-8"))
...
def test_named_circle_connection_origins_are_source_aware_and_access_neutral() -> None:
    migration = (MIGRATIONS_DIR / "134_one_location_circle_connection_origins.sql").read_text(
        encoding="utf-8"
    )
    schema = json.loads(SCHEMA_CONTRACT_PATH.read_text(encoding="utf-8"))
...
def test_targeted_circle_member_invites_require_acceptance_and_are_bounded() -> None:
    migration = (MIGRATIONS_DIR / "135_one_location_circle_member_invites.sql").read_text(
        encoding="utf-8"
    )
    schema = json.loads(SCHEMA_CONTRACT_PATH.read_text(encoding="utf-8"))
...
def test_named_circle_rollback_removes_only_the_additive_shape() -> None:
    rollback = (MIGRATIONS_DIR / "rollback" / "133_one_location_named_circles_down.sql").read_text(
        encoding="utf-8"
    )
    ...
    provenance_rollback = (
        MIGRATIONS_DIR / "rollback" / "134_one_location_circle_connection_origins_down.sql"
    ).read_text(encoding="utf-8")
    invite_rollback = (
        MIGRATIONS_DIR / "rollback" / "135_one_location_circle_member_invites_down.sql"
    ).read_text(encoding="utf-8")
