...
def test_regulated_runtime_guard_requires_ria_intelligence_in_production(monkeypatch):
    ...
    try:
        ...
        assert "RIA_INTELLIGENCE_VERIFY" in str(exc)
    else:
        raise AssertionError("Expected production runtime guard to require Stage 1 config")


def test_regulated_runtime_guard_rejects_prod_bypass(monkeypatch):
    monkeypatch.setenv("ENVIRONMENT", "production")
    monkeypatch.setenv("RIA_INTELLIGENCE_VERIFY_BASE_URL", "https://ria.example.com")
...
async def test_submit_ria_onboarding_reverifies_stage1_before_granting_access(monkeypatch):
    ...
    monkeypatch.setattr(service, "schedule_dossier_enrichment", lambda *_args, **_kwargs: None)
