...
def test_ria_intelligence_verifier_accepts_matching_crd_and_iard(monkeypatch):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert request.content == b'{"query":"Akash Katla"}'
    ...
    adapter = RIAIntelligenceVerificationAdapter(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(
                status_code=200,
                json=_stage1_payload(crd_number="7654321"),
            )
        )
    )
    result = _run(
...
def test_ria_intelligence_verifier_rejects_iard_mismatch(monkeypatch):
    ...
    adapter = RIAIntelligenceVerificationAdapter(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(
                status_code=200,
                json=_stage1_payload(sec_number="801-99999"),
            )
        )
    )
    result = _run(
...
def test_ria_intelligence_verifier_returns_provider_unavailable_when_stage1_lacks_iard(monkeypatch):
    monkeypatch.setenv("RIA_INTELLIGENCE_VERIFY_BASE_URL", "https://ria-intelligence.example")
    monkeypatch.delenv("RIA_INTELLIGENCE_VERIFY_URL", raising=False)
    ...
    adapter = RIAIntelligenceVerificationAdapter(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(
                status_code=200,
                json=_stage1_payload(sec_number=None),
            )
        )
    )
    result = _run(
...
def test_stage1_lookup_accepts_short_query_when_provider_returns_crd(monkeypatch):
    monkeypatch.setenv("RIA_INTELLIGENCE_VERIFY_BASE_URL", "https://ria-intelligence.example")
    monkeypatch.delenv("RIA_INTELLIGENCE_VERIFY_URL", raising=False)
    ...
    adapter = RIAIntelligenceStage1LookupAdapter(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(
                status_code=200,
                json=_stage1_payload(
                    full_name="Andrew Garrett Kirkland",
                    crd_number="7413463",
    ...
    result = _run(adapter.verify_name(query="Andrew"))

    assert result.status == "verified"
    assert result.matched_name == "Andrew Garrett Kirkland"
    assert result.crd_number == "7413463"
