...
|----------|------------|----------|-------|
| `HUSSH_ONE_POD_RELEASE_B64` | `hushh_mcp/services/pod_release.py` | Required for new approved upgrades | Base64-encoded public release manifest generated by the governed dev build, bound to `HUSSH_ONE_POD_IMAGE`, source SHA and workflow provenance. Missing or incompatible metadata disables new installation approval; it does not cancel durable recovery. See [pod release operations](../../../docs/reference/operations/dev-pod-first-light-runbook.md#release-metadata-and-compatibility). |
...
| `ONE_PUBLIC_PROFILE_DISCOVERY_ENABLED` | `hushh_mcp/services/public_profile_discovery_service.py` | Optional, default `false` | Global public-profile discovery gate. Keep off until migration, worker scheduler and provider access are verified. |
| `ONE_PUBLIC_PROFILE_DISCOVERY_ALLOWED_USER_IDS` | `hushh_mcp/services/public_profile_discovery_service.py` | Required for hosted cohort | Comma-separated Firebase UIDs enabled for the small release cohort; empty means local/development/test only. |
| `ONE_PUBLIC_PROFILE_DISCOVERY_DRAIN_ENABLED` | `api/routes/profile_discovery_work_drain.py` | Required for worker | Separately enables the OIDC-authenticated Cloud Scheduler queue drain; defaults to `false`. |
| `ONE_PUBLIC_PROFILE_DISCOVERY_DRAIN_AUDIENCE` | `hushh_mcp/services/scheduler_identity.py` | Required for hosted worker | Exact HTTPS backend origin expected in Cloud Scheduler OIDC tokens. |
| `ONE_PUBLIC_PROFILE_DISCOVERY_DRAIN_SCHEDULER_SERVICE_ACCOUNTS` | `hushh_mcp/services/scheduler_identity.py` | Required for hosted worker | Exact dedicated Cloud Scheduler service-account allowlist; empty refuses everyone. |
| `ONE_PUBLIC_PROFILE_DISCOVERY_DAILY_LIMIT` | `hushh_mcp/services/public_profile_discovery_service.py` | Optional, defaults to `1000` | Maximum new HusshOne scan-start requests per UTC database day; bounded to 100,000. |
| `INTELLIGENCE_API_BASE_URL` | `hushh_mcp/services/public_profile_discovery_service.py` | Required for enabled discovery | Server-side HusshOne API base URL used by the background worker. |
| `INTELLIGENCE_API_KEY` | `hushh_mcp/services/public_profile_discovery_service.py` | Secret when enabled | Server-side HusshOne API credential; store in Secret Manager and never expose it to the browser. |
