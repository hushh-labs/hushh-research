...
| `PUPPY_RELAY_RENDEZVOUS_URL` | `api/routes/one/puppy_relay.py` | Dev only | Existing Redis/Memorystore URL for short-lived device presence, busy fencing, and cross-instance relay frames. Falls back to `RATE_LIMIT_STORAGE_URI`; never stores prompt bodies or credentials. |
