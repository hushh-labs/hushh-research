...
| `GMAIL_OAUTH_REDIRECT_URI` | `hushh_mcp/services/gmail_receipts_service.py` | Yes (Gmail) | Environment-owned Gmail OAuth callback. It must equal `APP_FRONTEND_ORIGIN + /one/profile/gmail/oauth/return`; the key name is shared but the value is environment-specific. Gmail sign-in requests both read and send scopes; sending remains disabled in the app until the owner turns it on. |
...
| `GOOGLE_OAUTH_CLIENT_ID` | `hushh_mcp/services/google_connection_service.py` | Preferred for Google integrations | Google OAuth web-client id for Calendar, Drive, Contacts, and connected-Gmail sending. Falls back to `GMAIL_OAUTH_CLIENT_ID` during the compatibility transition. |
...
Kai portfolio import defaults to `KAI_PORTFOLIO_IMPORT_PRIMARY_MODEL = "gemini-3.7-flash"` in `hushh_mcp/constants.py`. Runtime can override the import model with `KAI_PORTFOLIO_IMPORT_MODEL` or `KAI_PORTFOLIO_IMPORT_PRIMARY_MODEL`; hosted provider/auth comes from Vertex workload ADC. Gemini 3.7 is pinned to the global Vertex endpoint and its generation config omits legacy sampling controls. Local API-key compatibility requires explicit `HUSHH_GENAI_AUTH_MODE=developer_api_key`.
