...
describe("sanitizeErrorMessage — sensitive pattern leakage prevention", () => {
  ...
  ])("redacts database detail from server error: %s", (rawMessage) => {
  ...
  ])("redacts file system path from server error: %s", (rawMessage) => {
    const result = sanitizeErrorMessage(err(rawMessage), 500);
    expect(result.message).not.toMatch(
...
describe("sanitizeErrorMessage — error category classification", () => {
  ...
  it("marks missing status as isClientError=false", () => {
    expect(sanitizeErrorMessage(err("unknown"), undefined).isClientError).toBe(
      false
    );
  });
});
...
// ---------------------------------------------------------------------------
// formatErrorResponse — dev-mode debug field guard
// ---------------------------------------------------------------------------
...
describe("formatErrorResponse — NODE_ENV dev-mode debug guard", () => {
  ...
  it("always includes error and category regardless of NODE_ENV", () => {
    vi.stubEnv("NODE_ENV", "test");
    const result = formatErrorResponse(err("something"), 400);
