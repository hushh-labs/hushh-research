 ...
 * the tree -- otherwise removing one note could silently archive every other
 * note that happened to share its wording.
 */
 ...
 * marks it archived rather than deleting it -- non-destructive: the record
 * stays, just no longer "active", with a note on where it moved to. Callers
 * MUST check the returned `matched` flag and fail the write if false --
 * otherwise a note can silently disappear from readable_highlights while
 * the underlying entity stays active, orphaning it.
 */
