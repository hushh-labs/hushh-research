...
export interface HoldingsMergeResult {
  holdings: AnyObj[];
  added: string[];
  averaged: string[];
  retained: string[];
...
/**
 * Merge newly imported holdings with existing holdings.
 *
 * - Duplicate symbols: average cost-basis-per-share and price using the
 *   newer import's quantity (the new statement is the source of truth for
 *   current positions). Market value, quantity, and unrealized figures come
 *   from the new import.
 * - New-only symbols: added as-is.
 * - Existing-only symbols: retained as-is (not erased).
 */
export function mergeWithExistingHoldings(
  existingHoldings: AnyObj[] | undefined,
  newHoldings: AnyObj[] | undefined
): HoldingsMergeResult {
  const normalizedExisting = consolidateHoldingsBySymbol(existingHoldings);
  const normalizedNew = consolidateHoldingsBySymbol(newHoldings);

  const existingMap = new Map<string, AnyObj>();
  for (const h of normalizedExisting) {
    const sym = String(h.symbol || "").trim().toUpperCase();
    if (sym) existingMap.set(sym, h);
  }

  const newMap = new Map<string, AnyObj>();
  for (const h of normalizedNew) {
    const sym = String(h.symbol || "").trim().toUpperCase();
    if (sym) newMap.set(sym, h);
  }

  const merged: AnyObj[] = [];
  const added: string[] = [];
  const averaged: string[] = [];
  const retained: string[] = [];

  for (const [sym, newH] of newMap) {
    const existH = existingMap.get(sym);
    if (!existH) {
      merged.push(newH);
      added.push(sym);
      continue;
    ...
    const existCostBasis = parseMaybeNumber(existH.cost_basis);
    const existQty = parseMaybeNumber(existH.quantity) ?? 0;
    const newCostBasis = parseMaybeNumber(newH.cost_basis);
    const newQty = parseMaybeNumber(newH.quantity) ?? 0;

    let avgCostBasis: number | undefined;
    if (existCostBasis !== undefined && newCostBasis !== undefined) {
      const existPerShare =
        Math.abs(existQty) > 1e-9 ? existCostBasis / existQty : 0;
      const newPerShare =
        Math.abs(newQty) > 1e-9 ? newCostBasis / newQty : 0;
      if (existPerShare > 0 && newPerShare > 0) {
        const avgPerShare = (existPerShare + newPerShare) / 2;
        avgCostBasis = avgPerShare * newQty;
      } else {
        avgCostBasis = newCostBasis;
      }
    } else {
      avgCostBasis = newCostBasis ?? existCostBasis;
    }

    const marketValue = parseMaybeNumber(newH.market_value) ?? 0;
    const unrealized =
      avgCostBasis !== undefined ? marketValue - avgCostBasis : parseMaybeNumber(newH.unrealized_gain_loss);
    let unrealizedPct: number | undefined;
    if (unrealized !== undefined && avgCostBasis !== undefined && Math.abs(avgCostBasis) > 1e-6) {
      unrealizedPct = (unrealized / avgCostBasis) * 100;
    }
    ...
    averaged.push(sym);
    existingMap.delete(sym);
  }

  for (const [sym, existH] of existingMap) {
    merged.push(existH);
    retained.push(sym);
  }

  return { holdings: merged, added, averaged, retained };
}
