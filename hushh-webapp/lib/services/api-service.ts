...
function getVoiceTransportMode(): VoiceTransportMode {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (backendHost && originHost && backendHost === originHost) {
      return {
        mode: 'direct_backend',
        reason: 'same_origin_default_direct',
        backendUrl: backend,
      }
    }
  }
  if (backendHost) {
    return {
      mode: 'direct_backend',
      reason: 'backend_url_default_direct',
      backendUrl: backend,
    }
  }
...
function normalizeVoiceAudioMimeType(
...
function prepareVoiceAudioUpload(data: {
  ...
  const mimeType = normalizeVoiceAudioMimeType(
    data.mimeType || rawMimeType || 'audio/webm'
  )
...
export interface InsightFeedbackAggregates {
...
export class ApiService {
  ...
  static async getKaiVoiceCapabilityJson(data: {
    ...
    const payload = (await response
      .json()
      .catch(() => ({}))) as Partial<VoiceCapabilityResponse>
    if (!response.ok) {
      const detail =
  ...
  static async revokeConsent(data: {
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        const response = new Response((e as Error).message || 'Failed', {
          status: 500,
        })
        trackEvent('consent_action_result', {
          action: 'revoke',
        ...
        const stream = new ReadableStream<Uint8Array>({
          async start(controller) {
            ...
            try {
              ...
              if (!sawTerminalEvent) {
                fail(
                  new Error('Native import stream ended without terminal event')
                )
                return
              }
              close()
  ...
  static async submitInsightFeedback(
    data: InsightFeedbackRequest
  ): Promise<InsightFeedbackResponse> {
    const authToken = await this.getFirebaseToken()
    if (!authToken) {
      throw new Error('Missing Firebase ID token for insight feedback')
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        if (
          ...
          !('aggregates' in payload)
        ) {
          throw new Error('Feedback response was incomplete.')
        }

        return payload as InsightFeedbackResponse
      } catch (error) {
        lastError = error
        if (
          attempt < maxAttempts &&
        ...
        throw error instanceof Error
          ? error
          : new Error('Failed to submit insight feedback.')
      }
    }
