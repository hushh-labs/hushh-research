...
const surfaceMapPath = path.join(appRoot, "frontend-native-surface-map.generated.json");
const gatewayPath = path.join(repoRoot, "contracts/kai/kai-action-gateway.vnext.json");
const outputs = [
...
function idFor(route) { return `route.${route === "/" ? "root" : route.slice(1).replace(/[^a-z0-9]+/gi, ".")}`; }
...
const routes = (surfaceMap.routes || []).map((entry) => {
  ...
  const actionIds = routeActions
    ...
    .sort();
  const delegateAgentIds = [...new Set(routeActions
  ...
  return {
    ...
    delegation_policy: {
      mode: authored.delegation_policy || (delegateAgentIds.length ? "one_action_gate" : "no_delegation"),
...
for (const output of outputs) {
  ...
  if (check) {
    if (!fs.existsSync(output) || fs.readFileSync(output, "utf8") !== next) {
      console.error(`route-orchestration-index: ${path.relative(repoRoot, output)} is stale.`);
      process.exitCode = 1;
    }
  } else {
...
if (!process.exitCode) console.log(check ? "Route orchestration index is current." : `Wrote route orchestration index (${routes.length} routes).`);
