...
import { Bot, Grip, Maximize2, Minimize2, Square } from "lucide-react";
        ...
        >
          ...
          >
            ...
            <AgentChatWorkspace
              variant="popover"
              windowControls={
                <AgentPopoverSizeControls sizeMode={sizeMode} setSizeMode={setSizeMode} />
              }
...
function AgentPopoverSizeControls({
  sizeMode,
  setSizeMode,
}: {
  sizeMode: AgentPopoverSizeMode;
  ...
  const controls: Array<{
    mode: Exclude<AgentPopoverSizeMode, "custom">;
    label: string;
    icon: ReactNode;
  }> = [
    {
  ...
  return (
    <div
      className="hidden items-center rounded-md border border-border/70 bg-background p-0.5 sm:flex"
      aria-label="Agent size"
      role="group"
    >
      {controls.map((control) => {
        const active = sizeMode === control.mode;
        return (
          <Button
            key={control.mode}
            type="button"
            ...
            size="icon-xs"
            onClick={() => setSizeMode(control.mode)}
