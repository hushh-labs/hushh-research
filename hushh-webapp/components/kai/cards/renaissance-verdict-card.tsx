...
import { TrendingDown, TrendingUp, Minus } from "@/components/icons";
...
function signalSummary(
  ...
  const dataQuality = row.degraded
    ? "Information quality is delayed, so One treats this as lower-confidence context."
    : null;
  ...
  const parts = [
    ...
    tier ? `Conviction tier: ${tier}.` : null,
    fcf ? `Free cash flow stands at ${fcf}.` : null,
    "One has no high-conviction directional signal from this list alone.",
    dataQuality,
  ].filter(Boolean);
  return parts.join(" ");
}
