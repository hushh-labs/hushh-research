...
function LineageNode({
  ...
  return (
    <div className="relative">
      <span
        className="absolute -left-[1.6rem] top-4 h-2 w-2 rounded-full bg-blue-500/60"
        aria-hidden
      />
        ...
        <div className="flex shrink-0 items-center gap-1">
          {canExpandInPlace ? (
            <button
              ...
              className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-blue-500/10 hover:text-foreground"
      ...
      {expanded ? (
        <div className="relative ml-3 mt-2.5 space-y-2.5 border-l-2 border-blue-500/15 pl-6">
...
export function CitationLineagePanel() {
  ...
  async function handleSaveToNotes() {
    ...
    try {
      ...
      const result = await PkmWriteCoordinator.saveMergedDomain({
        ...
        vaultKey,
        vaultOwnerToken,
        confirmation: { confirmedByUser: true, surface: "web", source: "sage_citation" },
        build: (context) => ({
          domainData: addNoteEntity(context.currentDomainData, noteText, "sage_citation"),
          mergeDecision: { merge_mode: "replace_domain", target_domain: saveDomain },
          summary: {
  ...
  async function handleAddToThread() {
    ...
    try {
      ...
      const result = await PkmWriteCoordinator.saveMergedDomain({
        ...
        vaultKey,
        vaultOwnerToken,
        confirmation: { confirmedByUser: true, surface: "web", source: "sage_citation_thread" },
        build: (context) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-blue-500/25 bg-card/85 p-5 shadow-[0_1px_2px_rgba(15,23,42,0.06)] sm:p-6">
        ...
        <div className="mt-4 flex flex-col gap-2.5 sm:flex-row">
          ...
          <Button
            ...
            className="h-12 bg-blue-600 px-6 text-base text-white hover:bg-blue-700 sm:w-auto"
          >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {EXAMPLE_QUERIES.map((example) => (
              <button
                ...
                className="rounded-full border border-border/60 bg-background/60 px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:border-blue-500/40 hover:text-foreground"
              >
                {example}
              </button>
            ))}
          ...
          <div className="mt-4 space-y-2.5">
            {candidates.map((paper) => (
              <button
                ...
                className="flex w-full items-center justify-between gap-3 rounded-xl border border-border/60 bg-background/60 p-4 text-left transition-colors hover:border-blue-500/40"
              >
          ...
          {trail.map((paper, index) => {
            ...
            return (
              <span key={paper.id} className="flex items-center gap-2">
                ...
                {isCurrent ? (
                  <span className="max-w-[18rem] truncate rounded-full bg-blue-500/15 px-3 py-1.5 font-medium text-blue-700 dark:text-blue-300">
                    {paper.title}
                  </span>
                ) : (
                  <button
                    type="button"
                    onClick={() => void openPaper(paper)}
                    className="max-w-[18rem] truncate rounded-full border border-border/60 bg-card px-3 py-1.5 text-muted-foreground hover:border-blue-500/40 hover:text-foreground"
                  >
                    {paper.title}
                  </button>
                )}
      ...
      {loadingLineage ? (
        <div className="space-y-2.5 border-l-2 border-blue-500/10 pl-6">
      ...
      {lineage && !loadingLineage ? (
        <>
          ...
          {lineage.references.length === 0 ? (
            ...
            <div className="space-y-2.5 border-l-2 border-blue-500/20 pl-6">
              {(() => {
          ...
          <div className="flex justify-center">
            <div className="h-6 w-0.5 bg-gradient-to-b from-blue-500/20 to-blue-500/40" aria-hidden />
          </div>

          <div className="relative overflow-hidden rounded-2xl border border-blue-500/25 bg-card/85 p-5 sm:p-6">
            <span className="text-xs font-medium uppercase tracking-wide text-blue-700/80 dark:text-blue-300/80">
              Currently tracing
            </span>
            ...
            <div className="mt-3.5 flex items-start gap-2 border-t border-emerald-500/15 pt-3.5 dark:border-emerald-400/15">
          ...
          <div className="flex justify-center">
            <div className="h-6 w-0.5 bg-gradient-to-b from-blue-500/40 to-blue-500/20" aria-hidden />
          </div>
