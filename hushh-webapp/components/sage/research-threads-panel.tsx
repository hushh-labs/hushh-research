...
function KeyTermChips({ items }: { items: ResearchThreadKeyTerm[] }) {
  ...
  return (
    <div className="mt-2.5 flex flex-wrap items-center gap-1.5 border-t border-emerald-500/10 pt-2.5">
      {items.map((item, index) => (
        <Popover key={index}>
          <PopoverTrigger className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/[0.06] px-2 py-0.5 text-xs font-medium text-emerald-700 transition-colors hover:border-emerald-500/50 dark:text-emerald-300">
            {item.term}
          </PopoverTrigger>
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card/85 p-4">
      <span className="flex items-center gap-2 text-sm font-semibold text-foreground">
        <Tag className="h-4 w-4 text-emerald-700 dark:text-emerald-300" aria-hidden />
        Glossary
      </span>
      <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
        {glossary.map((item, index) => (
          <Popover key={index}>
            <PopoverTrigger className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/[0.06] px-2.5 py-1 text-xs font-medium text-emerald-700 transition-colors hover:border-emerald-500/50 dark:text-emerald-300">
              {item.term}
            </PopoverTrigger>
            <PopoverContent className="w-64 text-sm">
              <p className="font-medium text-foreground">{item.term}</p>
...
function RelatedThreadsCard({
  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card/85 p-4">
      <span className="flex items-center gap-2 text-sm font-semibold text-foreground">
        <Link2 className="h-4 w-4 text-blue-700 dark:text-blue-300" aria-hidden />
        Related threads
      </span>
      <div className="mt-2.5 space-y-2">
        {related.slice(0, 4).map((r) => (
          <button
            ...
            className="flex w-full flex-col gap-1 rounded-lg border border-border/50 bg-background/60 p-2.5 text-left transition-colors hover:border-blue-500/40"
          >
...
const sageComparisonChartConfig = {
  value: {
    label: "Value",
    color: "rgb(16 185 129)", // emerald-500 -- these are numbers the AI answer itself stated
  },
...
function ComparisonBars({ items }: { items: ResearchThreadComparison[] }) {
  ...
  return (
    <div className="mt-2.5 space-y-3 border-t border-emerald-500/10 pt-2.5">
...
function NewThreadBox({
  ...
  return (
    <div className="rounded-2xl border border-blue-500/25 bg-card/85 p-5 shadow-[0_1px_2px_rgba(15,23,42,0.06)] sm:p-6">
 ...
 * (a domain key registered in domain_contracts.py -- the PKM domain
 * system is fully dynamic, no backend schema change needed) via the same
 ...
 * turns/tracedPapers/synthesis structure. `replace_domain` tells the
 * coordinator to persist our already-correct read-modify-write result
 * verbatim instead.
 */
export function ResearchThreadsPanel() {
   ...
   * triggers goes through a fire-and-forget dynamic import that isn't
   * awaited by the write path, so a stale-first read immediately after a
   * write can race it and return pre-write data -- e.g. a just-created
   * thread not yet in `threads`, making the detail view think it doesn't
   * exist.
   */
   ...
   * recent question, in ONE combined write. Order inside the write matters:
   * papers are added BEFORE the synthesis timestamp is set, so
   * synthesis.generatedAt always lands >= the paper-add's updated_at bump
   * within this same write -- otherwise the staleness check below would see
   * its own write as stale and loop.
   */
  async function enrichThreadAfterTurn(
    ...
    try {
      ...
      // traced_papers:30) on the backend -- most-recent turns kept since
      // synthesis cares about current state, not the earliest questions asked.
      ...
      await PkmWriteCoordinator.saveMergedDomain({
        ...
        vaultKey,
        vaultOwnerToken,
        confirmation: { confirmedByUser: true, surface: "web", source: "sage_research_thread_enrich" },
        build: (context) => {
          ...
          return {
            domainData: updated,
            mergeDecision: { merge_mode: "replace_domain", target_domain: RESEARCH_DOMAIN },
  ...
  async function handleThreadCreated(
    ...
    const result = await PkmWriteCoordinator.saveMergedDomain({
      ...
      vaultKey,
      vaultOwnerToken,
      confirmation: { confirmedByUser: true, surface: "web", source: "sage_research_thread" },
      build: (context) => {
  ...
  async function handleArchiveThread(threadId: string) {
    ...
    await PkmWriteCoordinator.saveMergedDomain({
      ...
      vaultKey,
      vaultOwnerToken,
      confirmation: { confirmedByUser: true, surface: "web", source: "sage_research_thread_archive" },
      build: (context) => {
  ...
  async function handleRemoveTracedPaper(threadId: string, paperId: string) {
    ...
    await PkmWriteCoordinator.saveMergedDomain({
      ...
      vaultKey,
      vaultOwnerToken,
      confirmation: { confirmedByUser: true, surface: "web", source: "sage_research_thread_remove_paper" },
      build: (context) => {
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-start gap-3">
            <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-500/10 text-blue-700 dark:text-blue-300">
              <FlaskConical className="h-5 w-5" aria-hidden />
            </span>
                ...
                return (
                  <div key={index} className="space-y-2.5">
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="min-w-0 flex-1 rounded-2xl rounded-tl-sm border border-emerald-500/15 bg-emerald-500/[0.03] px-3.5 py-2.5 dark:border-emerald-400/15">
                          ...
                          <div className="mt-2.5 flex flex-wrap gap-1.5 border-t border-emerald-500/10 pt-2.5">
                            {turn.sources.map((source) => (
                              <a
                                ...
                                className="inline-flex items-center gap-1 rounded-full border border-border/60 bg-card px-2 py-0.5 text-xs text-muted-foreground transition-colors hover:border-blue-500/40 hover:text-foreground"
                              >
                                {source.title}
          ...
          {thread.tracedPapers.length === 0 ? (
            ...
            <div className="mt-3 space-y-3">
              ...
              {thread.tracedPapers.map((paper) => (
                <div
                  key={paper.id}
                  className="group relative rounded-lg border border-border/50 bg-background/60 p-3 transition-colors hover:border-blue-500/40"
                >
                  <button
                    type="button"
          ...
          {activeThreads.map((thread) => {
            ...
            return (
              <button
                ...
                className="flex flex-col gap-3 rounded-xl border border-border/60 bg-card/85 p-4 text-left shadow-[0_1px_2px_rgba(15,23,42,0.06)] transition-colors hover:border-blue-500/40"
              >
                <div className="flex items-start gap-3">
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-500/10 text-blue-700 dark:text-blue-300">
