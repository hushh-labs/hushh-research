  ...
  async function handleSave(resultId: string) {
    ...
    try {
      ...
      const writeResult = await PkmWriteCoordinator.saveMergedDomain({
        ...
        vaultKey,
        vaultOwnerToken,
        confirmation: { confirmedByUser: true, surface: "web", source: "sage_research" },
        build: (context) => ({
          domainData: addNoteEntity(context.currentDomainData, noteText, "sage_research"),
          mergeDecision: { merge_mode: "replace_domain", target_domain: effectiveSaveDomain },
          summary: {
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {suggestedPrompts.map((prompt) => (
            <button
              ...
              disabled={asking}
              className="rounded-full border border-border/60 bg-card px-3 py-1.5 text-left text-xs text-muted-foreground transition-colors hover:border-emerald-500/40 hover:text-foreground disabled:opacity-60"
            >
              {prompt}
            </button>
          ))}
      ...
      <button
        ...
        className={cn(
          ...
          challengeMode
            ...
            : "border-border/60 bg-card text-muted-foreground hover:border-blue-500/40 hover:text-foreground",
        )}
