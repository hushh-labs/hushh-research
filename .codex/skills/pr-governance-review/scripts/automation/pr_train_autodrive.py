    ...
    for idx,ch in enumerate(chunks):
        ...
        if not os.path.exists(out):
            run(["python3",ENGINE,"--repo",REPO,"--prs",",".join(map(str,ch)),
                 "--repass-after-changes","--json","--output",out])
...
def post_record(n,body):
    if not body or "## Changes Requested" not in body: return "no_template"
    rc,o,e=run(["gh","api",f"repos/{REPO}/pulls/{n}/reviews"]); rid=None; ex=None
    if rc==0:
        try:
            for rv in json.loads(o):
                if (rv.get("user") or {}).get("login") in MAINT and rv.get("state")=="CHANGES_REQUESTED":
                    rid=rv["id"]; ex=rv.get("body")
        except Exception: pass
    if ex is not None and norm(ex)==norm(body): return "already"
    if rid: rc2,o2,e2=run(["gh","api","-X","PUT",f"repos/{REPO}/pulls/{n}/reviews/{rid}","-f",f"body={body}"])
    else: rc2,o2,e2=run(["gh","pr","review",str(n),"--repo",REPO,"--request-changes","--body",body])
...
def main():
    ...
    for n in sorted(reps):
        ...
        r=post_record(n,reps[n]["comment"])
