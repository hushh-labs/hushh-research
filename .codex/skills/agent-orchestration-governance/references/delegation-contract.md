...
Product-agent delegation (One delegating to Kai/Nav/KYC/email/location specialists at runtime) is governed by the Project-Wide Agent Architecture Doctrine in `AGENTS.md`: statefulness decided by runtime topology (the shared hub stays dumb by default with context flowing in through consented state; a per-user pod is an intelligent private agent holding its own encrypted memory), delegation as a non-breaking wrapped function of the existing dispatch contract with per-hop scoped encrypted exports, ADK Task API as the substrate once a hop crosses process or network boundaries, subsystem-owned shared state behind replaceable seams (including existing configurable Redis rate limits and caching), and exactly one routing authority per surface. This file governs the ENGINEERING delegation lanes (Codex subagents); do not conflate the two, but keep both aligned with that doctrine.
