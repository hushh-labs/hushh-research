...
Product-agent delegation (One delegating to Kai/Nav/KYC/email/location specialists at runtime) is governed by the Project-Wide Agent Architecture Doctrine in `AGENTS.md`: shared-runtime context flows in through owner-scoped consented state, while a private pod has a separate owner-isolated recovery topology. Delegation remains a non-breaking wrapped function of the existing dispatch contract with per-hop scoped encrypted exports, ADK Task API is the preferred substrate once a hop crosses process or network boundaries, and each surface has exactly one routing authority. This file governs the ENGINEERING delegation lanes (Codex subagents); do not conflate the two, but keep both aligned with that doctrine.
...
The TOML files under `.codex/agents` are the authored fleet. Claude Code needs a
