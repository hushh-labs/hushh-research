...
Product-agent delegation (One delegating to Kai/Nav/KYC/email/location specialists at runtime) is governed by the Project-Wide Agent Architecture Doctrine in `AGENTS.md`. Shared-runtime context comes from owner-scoped, consented turn/session state, A2A task payloads, and scoped encrypted exports; the private pod has separate owner-isolated encrypted recovery and explicit recall. Delegation wraps the existing dispatch contract without breaking it. The doctrine specifies ADK modes within a runtime, official A2A Tasks across process or deployment boundaries, MCP for consented tools and resources, subsystem-owned shared state behind replaceable seams, and one routing authority per surface. This file governs ENGINEERING delegation lanes (Codex subagents); do not conflate them with product agents.
...
## Shared Root Inheritance

Every parent session and child agent inherits both the Principal Craft Kernel
and Bacterial Software Architecture Gate in `AGENTS.md`.
...
2. Custom-agent TOML files should contain a short root-contract inheritance hook plus role-specific priorities, not duplicated doctrine.
...
3. `agents/*.toml`: the skill block that says which skills each read-only agent can support.
...
The TOML files under `agents` are the authored fleet. Claude Code needs a
markdown agent file before the same lane can be delegated to, so the markdown
side is generated by `sync_claude_agents.py --write` and verified by `--check` in
`scripts/ci/repo-governance-check.sh`.

1. Edit the TOML, never a generated mirror; each mirror carries a generated-from marker.
2. Mirrors inherit the session model. Pin a model only when the TOML states a reason.
3. Read-only sandbox maps to a tool list without `Edit` or `Write`: a lane that needs
   a file changed returns the change to the parent rather than making it.
