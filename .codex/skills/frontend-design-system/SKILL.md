...
14. Form controls have an explicit shared contract: `Input`, `InputGroup`, `Textarea`, `SelectTrigger`, `CommandInput`, and combobox field shells use `--app-input-radius`; compound controls keep inner inputs square. Use `--app-form-field-gap`, `--app-form-related-gap`, and `--app-form-section-gap` for label, related-action, and section rhythm. A secondary recovery link may be visually small but must retain a transparent 44px hit area. Related-method groups keep the descriptor on one line and a centered action row on the next inside a capped form measure; keep visible text close to the descriptor and avoid pill padding, `self-start` offsets, or left-anchored rows. When a hard-gated escape group is also present, use centered columns with one vertically centered divider; if only one group exists, omit the divider and keep it centered. In a hard-gated credential flow, place Recovery key beside Sign out in the same quiet escape group on the action row.
