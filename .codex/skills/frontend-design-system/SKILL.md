...
19. In-page content swaps (wizard steps, tab panels, step carousels) use the shared `.motion-step-enter` utility in `app/globals.css` (motion tokens: `--motion-duration-lg` + `--motion-ease-decelerate`, reduced-motion safe). Never use ad-hoc tailwind `animate-in fade-in slide-in-* duration-N` utilities for step/tab enters; those fragment the app's single motion frame.
20. Dark-mode chrome tints (bar glass, chrome-glass-surface, morphy-app-bg, route surface backgrounds) must derive from the live theme background via `color-mix(in oklab, var(--background) N%, transparent)`, never hardcoded `rgb(28,28,30)`/`#1c1c1e` panels. A hardcoded dark tint is LIGHTER than the true `--background` and reads as a milky band or a "double card" partition (the /one/kai regression). Route-level surface palettes (`--one-bg`, `--one-card`, `--one-surface`) must map to `var(--background)` and background-derived elevations.
21. Market/insight visualizations must be honest: no fabricated chart shapes. Sparklines render ONLY from a real series (scaled to its own min/max); degraded rows show a `Delayed` chip and a flat dashed baseline, never an invented always-green path. Bottom-sheet content (vault unlock and other Drawer flows) must not paint a second opaque card inside `DrawerContent`; the sheet is the single surface (morphy `Card effect="fill"` needs `!bg-transparent`).
## Handoff Rules
