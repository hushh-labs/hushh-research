...
10. Gmail-safe dense information tables must live in an inline-styled horizontal scroll wrapper with a fixed/minimum table width. Do not squeeze holdings or financial tables into mobile width if that causes overlap, truncation, or unreadable numeric columns.
11. Signed-in Email Helper, PKM, and consent UI proof must use the reviewer-mode vault-owner identity resolved from runtime env. Do not treat counterparty labels, copied recipients, thread ids, or admin fixtures as the active information owner.
...
14. Form controls have an explicit shared contract: `Input`, `InputGroup`, `Textarea`, `SelectTrigger`, `CommandInput`, and combobox field shells use `--app-input-radius`; compound controls keep inner inputs square. Use `--app-form-field-gap`, `--app-form-related-gap`, and `--app-form-section-gap` for label, related-action, and section rhythm. A secondary recovery link may be visually small but must retain a transparent 44px hit area. Credential fallback groups keep the “Can’t get in?” descriptor on one centered line and all available quiet actions on the next centered row inside a capped form measure; separate multiple actions with middle-dot separators and use no second primary CTA, pill, column, or vertical divider. If only one action is available, keep it as the single centered element without a separator. Use `--app-form-section-gap` for the larger pause after the primary unlock action and `--app-form-related-gap` to keep the descriptor close to its action row. Keep visible text close to the descriptor and avoid pill padding, `self-start` offsets, or left-anchored rows. Recovery, alternate unlock, and Sign out share the same quiet escape row when present.
