...
LOCATION_COMMAND_RUNTIME_ENABLED="${LOCATION_COMMAND_RUNTIME_ENABLED:-false}"
LOCATION_COMMAND_RUNTIME_ENABLED="$(printf '%s' "$LOCATION_COMMAND_RUNTIME_ENABLED" | tr '[:upper:]' '[:lower:]')"
case "$LOCATION_COMMAND_RUNTIME_ENABLED" in
  1|true|yes|on)
    LOCATION_COMMAND_RUNTIME_ENABLED="true"
    ;;
  0|false|no|off|"")
    LOCATION_COMMAND_RUNTIME_ENABLED="false"
    ;;
  *)
    echo "LOCATION_COMMAND_RUNTIME_ENABLED must be a boolean." >&2
    exit 1
    ;;
esac
