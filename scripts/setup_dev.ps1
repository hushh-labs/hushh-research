...
# ---------------------------------------------------------------------------
# Step 4 — Schema → TypeScript export (keeps frontend types in sync)
# ---------------------------------------------------------------------------
Write-Host ""
Write-Step "Exporting Pydantic schemas to TypeScript (Beast Mode type sync)..."

$SchemasFile = Join-Path $RepoRoot "consent-protocol\schemas.py"
$ExportScript = Join-Path $RepoRoot "scripts\export_schemas.py"

if (Test-Path $SchemasFile) {
    Push-Location (Join-Path $RepoRoot "consent-protocol")
    try {
        uv run python "..\scripts\export_schemas.py"
        Write-Ok "TypeScript types regenerated: hushh-webapp/lib/consent/consent-approval-payload.generated.ts"
    } catch {
        Write-Fail "Schema export failed: $_"
        Pop-Location
        exit 1
    }
    Pop-Location
} else {
    Write-Host "    [~] consent-protocol/schemas.py not found — skipping type export." -ForegroundColor DarkYellow
    Write-Host "        Merge PR #986 (consent-protocol Pydantic validation) first." -ForegroundColor DarkYellow
}

# ---------------------------------------------------------------------------
# Step 5 — CI parity: run the monorepo protocol stage locally
# ---------------------------------------------------------------------------
if (-not $LintOnly) {
    Write-Host ""
