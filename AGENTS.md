...
1. Statefulness follows runtime topology; owner isolation is permanent. In this shared runtime, an agent is a system prompt plus declared hands and tools. It holds no ambient cross-owner memory or privileged information access. Context flows in per turn through owner-scoped, consented state (session state keys, A2A task payloads, scoped encrypted exports). A private pod may have owner-isolated encrypted recovery state and explicit recall under its separate topology; that direction does not grant memory to the shared runtime or make the pod's working copy a second PKM authority. Protected information always arrives through a consent-gated channel, never by reaching around the trust boundary.
