...
9. Intelligence owns semantic assessment, and deterministic behaviour is authored in the agent's system instruction rather than reimplemented in host code. The active route and top authored interaction layer bound that assessment. Deterministic policy may validate, normalize, reject, enforce authority and consent, and catch a model FAILURE -- a timeout, malformed output, or schema violation. On a successful model call it must not do any of three things: compute an outcome the agent's own output contract declares (**decides instead**), overwrite or re-derive a field the agent returned with a non-empty value (**discards**), or route around the agent stage so the model is never asked (**skips**). It must not replace agent meaning with keyword or regex classification, infer DOM controls, or substitute a different action. A stage that is skipped, or a field that is substituted, is recorded as such in the execution trace and drift flags, because an unobservable substitution is indistinguishable from a model answer and makes every downstream metric read healthy exactly when the intelligence is absent. Measured 2026-09-11: changing one rule's wording moved a domain-creation decision from 0 of 10 to 8 of 10 on the same input, and the restrictive wording filed a person's communication style under the financial-advisor domain. The full contract, its one legitimate exception, and the declaration every new semantic surface must file, live in `consent-protocol/docs/reference/backend-semantic-boundary.md`. Lanes cite it; they never copy it.
