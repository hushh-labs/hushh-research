...
- Every recorded call appends one JSON line to
  `hushh-desktop/backend/tmp/fmp_call_budget/<UTC-date>.jsonl` (gitignored
  `tmp/`, never committed - local/UAT-runtime study data only).
...
Ported from consent-protocol/hushh_mcp/services/fmp_call_budget.py (the
canonical upstream source) - hushh-desktop's copy of this file did not
previously exist, so its FMP calls had no budget tracking at all.
"""
