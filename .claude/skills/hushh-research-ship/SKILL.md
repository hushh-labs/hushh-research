...
**Confirmed 2026-08-28: `enforce_admins` is `true`** (checked from
`ankitkumarsingh1702`, an admin). Alongside it: `reviews=1`, `last_push=true`,
13 bypass users, bypass team `allowed-maintainers-to-approve`. That earlier
reading settles the question this section used to leave open, and the answer
retires the lift rather than justifying it: `enforce_admins` was `true` *and*
`--admin` merged anyway, so being blocked at `BLOCKED` / `REVIEW_REQUIRED` is not
evidence that a lift is needed.
...
rather than reinstating the lift as the default. Two independent runs (2026-08-21
non-admin, 2026-08-28 admin) have now merged without it.
