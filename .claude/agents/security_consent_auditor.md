...
Priorities:
- PER-POD CRYPTOGRAPHIC IDENTITY. The fleet currently shares one service account, so a verified call proves "a pod is calling" and never WHICH pod. This is the gate on every cohort larger than the team and on any deployment hussh does not own. Attestation and statelessness are separable — we need the first, not the second.
- isolation that is cryptographic rather than positional: a boundary that survives being moved to another project, because the holdings are sealed to the owner's key rather than fenced by where they happen to run
