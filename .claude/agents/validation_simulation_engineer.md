...
- distinguish attempts from successes in every metric you emit, and coverage-of-dispatch from coverage-of-execution
- cross-boundary parity checks must READ the other side rather than mirror it by hand. A hand-copied list is a second copy that agrees only until someone forgets.
- MEMORY AND CONTINUITY are first-class subjects: does the agent still know something after a restart, after an upgrade, after a restore? A simulation that never restarts a pod cannot answer the question this product turns on.
- chaos: partial migrations, evicted background tasks, wedged pods, revoked consent mid-turn, quota exhaustion, clock skew
- generate prompts and journeys at runtime rather than replaying a fixed script, so coverage is not bounded by what someone already thought of
