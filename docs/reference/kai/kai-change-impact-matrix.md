...
| Kai Market News | `consent-protocol/api/routes/kai/market_insights.py`, `hushh-webapp/lib/kai/kai-market-news-resource.ts`, `hushh-webapp/components/kai/kai-market-news-page.tsx` | `/api/kai/market/news/(baseline/){user_id}` snapshot envelope (`items/next_cursor/has_more/snapshot_id/cache`) | duplicate pages, cursor conflict loops, page taps causing provider fanout | keep the cursor snapshot contract; remove only the route/UI consumer, never fall back to Analysis |
