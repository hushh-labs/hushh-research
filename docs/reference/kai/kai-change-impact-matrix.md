...
| Kai weight evaluation governance | `consent-protocol/hushh_mcp/services/kai_weight_eval_service.py`, `consent-protocol/hushh_mcp/services/personal_knowledge_model_service.py`, `consent-protocol/scripts/local_kai_weight_eval_regression.py` | offline candidate eval runs + gate decisions + audit projections (`kai_weight_eval_v1`, `kai_weight_eval_promotion_v1`) | no receipts loaded, unmatched outcomes spike, promotion artifacts missing, gate drift | keep candidate-only mode, block promotion on failed gates, verify audit projections before enabling broader rollout |
...
- Weight-eval projection schema changes documented in `docs/reference/kai/kai-weight-eval-scaffold.md`
