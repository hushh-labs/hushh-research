...
| `navigate_back` | Existing app back handler | Maps to `nav.back`. |
...
`switch_persona` is not exposed in the backend OpenAI tool schema. It can still be generated by frontend grounding from an authored gateway workflow, currently for `nav.ria_home`, where the workflow asks for RIA persona confirmation before routing to `/ria`.
...
| --- | --- | --- |
| `nav.kai_analysis` | wired route `/kai/analysis` | Can navigate to analysis. |
| `nav.analysis_history` | wired Kai command `history` | Can open analysis history. |
...
| `nav.profile` | wired Kai command `profile` | Can open `/profile`. |
| `nav.profile_gmail_panel` | wired route `/profile?panel=gmail` | Can open the Gmail profile panel. |
| `nav.profile_support_panel` | wired route `/profile?tab=account&panel=support` | Can open support panel. |
| `nav.profile_security_panel` | wired route `/profile?tab=privacy&panel=security` | Can open security panel. |
| `nav.profile_pkm_agent_lab` | wired route `/profile/pkm-agent-lab` | Can open PKM Agent Lab. |
...
| `nav.profile_receipts` | wired route `/profile/receipts` | Can open receipts memory page. |
...
| `nav.ria_home` | wired route `/ria` plus workflow | Can ask to switch to RIA persona, then route to `/ria`. |
| `nav.consents` | wired Kai command `consent` | Can open consent center. |
| `nav.back` | wired voice tool `navigate_back` | Can call the app back handler. |
| `nav.kai_dashboard` | wired Kai command `dashboard` | Can open `/kai/portfolio`. |
| `nav.kai_import` | wired Kai command `import` | Can open `/kai/import`. |
| `nav.kai_investments` | wired route `/kai/investments` | Can navigate to investments. |
| `nav.kai_optimize` | wired route `/kai/optimize` | Can navigate to optimize; backend command path can also execute `optimize`. |
| `nav.kai_home` | wired Kai command `home` | Can open `/kai`. |
...
- Portfolio dashboard: `kai.portfolio.share_pdf`, `kai.portfolio.optimize`, `nav.investments`, `kai.portfolio.connect_plaid`, `kai.portfolio.import_statement`, `kai.portfolio.delete_imported_data`, `kai.portfolio.refresh_plaid`
- Kai market: `kai.market.refresh`, `kai.market.switch_pick_source`, `nav.portfolio`
- Profile: `nav.profile_my_data`, `nav.profile_access`, `nav.profile_account`, `nav.profile_gmail`, `nav.profile_support`
...
`hushh-webapp/app/ria/page.voice-action-contract.json` defines `nav.ria_home`, but `app/ria/page.tsx` does not publish `usePublishVoiceSurfaceMetadata(...)`. The action can still work through route/workflow grounding, but live screen metadata is not symmetrical with the contracted surface.
