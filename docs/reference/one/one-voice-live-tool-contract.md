...
- Connections: the people tools bind to the Connect family's own gateway actions (`connect.search_people` for `resolve_person`/`confirm_person`, `connect.send_request`, `connect.accept_request`, `connect.reject_request`, `connect.cancel_request`, `connect.remove_connection`); nothing on the voice path binds to `location.find_contacts` (device contact sync) or calls contact sync to resolve a name. `resolve_person` searches display names only (a phone number or email is refused with `identifier_not_a_name`), reads up to three directory pages and reports `truncated` rather than claiming "nobody" or "the only match" past that; every offer carries a revision and a 10-minute expiry, and `confirm_person` refuses a stale offer. A new `resolve_person`/`resolve_circle` cancels every open card (a correction never leaves an approvable card for the previous target), and the relay tells the client and the model about every superseded card. `accept_connection_request`, `decline_connection_request` and `cancel_connection_request` act only on request ids the server listed in this conversation (`list_people`), refuse anything else as `request_not_offered`, and their cards name the request's real counterpart from that listing, never a name the model supplied. `list_people` reads one page of connections (`LIST_PAGE_LIMIT`) with `has_more`; `counts` carry the totals. `invite_person` (message up to 1000 characters, the connections route's own bound) decides from the *committed* payload: `sent` needs a new pending request whose requester is the person, an existing request in either direction is `already_pending` with its direction, and a missing id/status is `malformed_request_result`, never "sent". `accept_connection_request` on a scope-bearing request returns `scope_review_required` and an `open_request_review` client step that opens the Consent Center for that request; opening it accepts nothing, and the relay settles the real outcome (`accepted`, `still_pending`, `declined`, `withdrawn`, `review_unverified`) from a re-read after the screen reports back. `remove_connection` re-reads the relationship after the cascade and reports `unverified` if it still shows connected.
- Actor proof: the client fetches a fresh Firebase proof for the auth frame on every connect, and every `firebase_plane` confirmation -- spoken, tapped, or over HTTP -- re-verifies the Firebase proof (signature, expiry, revocation, and that it names the signed-in user) immediately before the pending row is confirmed. A refused proof leaves the row pending (`firebase_proof_required` / `firebase_proof_invalid`); a tap with a fresh proof completes it. A post-confirmation exception is narrated as an unknown outcome, never "nothing changed".
...
| People / Connections | "send Meera a connection request" (resolution first, then its own confirmation), "who am I connected with?", "is Kushal on Hussh?", "accept Rahul's request", "cancel my request to Dev", "disconnect from Ayesha" (ends the connection everywhere; not a circle removal, no block) | `resolve_person`, `confirm_person`, `list_people`, `get_person`, `invite_person`, `accept_connection_request`, `decline_connection_request`, `cancel_connection_request`, `remove_connection` |
