...
| Cards specialist | `agent_cards` | Payment-cards conversation over client-executed actions (`cards.list`, `cards.add`, `cards.reveal`); an AgentTool child of One, roster-gated on `ONE_PAYMENT_CARDS_ENABLED`, with no server-side tools, no PKM context injection, and no A2A transport. Card secrets live in the reserved `payment_cards` PKM domain and decrypt only in the owner's browser | `agent.cards.manage` invocation only; data leaves the vault solely through owner-approved `attr.payment_cards.summary.*` / `attr.payment_cards.secrets.*` grants |
