...
| `One` | One Voice wrappers, One product shell direction, Kai-era compatibility action runtime | The approved top relationship layer for greetings, memory, notifications, cross-domain help, and specialist handoffs | vision docs, future roadmap, founder briefs, shell/copy governance, One Voice docs | code paths and generated contract names unless the checked-in surface is One-owned | `One (direct voice surface and approved top private-agent layer; several action/planner identifiers remain Kai-era compatibility surfaces)` |
| `Agent One` | One, One Voice, future One relationship layer | Founder-facing name for the user's top private agent, not a second product ontology | founder story, product narrative, future-roadmap framing | code/runtime docs unless mapped to checked One-owned routes or wrappers | `Agent One / One (approved direction for the private agent the user owns; current implementation is partially One-owned and partially Kai-era compatibility)` |
| `Kai` | Kai finance surfaces, generated Kai action gateway, portfolio/market/analysis services | The current shipped finance specialist that One summons for finance workflows | investor/RIA product docs, finance workflow docs, architecture narratives | code paths, route docs, generated contract names, package-local implementation docs | `Kai (implemented today as the finance specialist across portfolio, market, analysis, RIA, and generated action compatibility surfaces)` |
...
| `Your agents. Yours to own.` | One/Kai/Nav ontology, BYOK, consent, PKM, scoped export | Durable product line for user-owned agent relationships | vision docs, community updates, founder briefs | runtime docs that need exact implementation proof | `Your agents. Yours to own. (product thesis; current implementation is partially One-owned and still uses Kai-era compatibility identifiers)` |
| `Personal Operating Layer` | One, Kai, Nav, KYC, generated action gateway, consented specialist handoff | The product model where One owns the relationship and specialists perform bounded craft | vision docs, founder briefs, future roadmap, product narratives | implementation docs unless the runtime surface is checked in | `Personal Operating Layer (north-star model for One plus specialist handoffs; current implementation is partial and bounded by checked-in compatibility contracts)` |
...
| --- | --- | --- |
| `Hussh is your private MCP server and AI agent.` | Blurs platform infrastructure with the user's private agent. | `Hussh is the platform and trust infrastructure. One is the private agent.` |
...
7. `One` is capitalized when it refers to the top private agent and is not described as fully shipped until the runtime proves it.
