...
| POST | `/api/one/location/circle-member-invites` | VAULT_OWNER Bearer | Active-member batch ADD of the caller's selected direct connections: every person named must already be an active connection of the actor, so membership is written outright rather than invited, and each is notified by name. Actor identity comes only from the token. Non-owners may have at most five people they added still active in a Circle they do not own, terminal invitees keep the 12-hour Circle-wide cooldown so a direct add cannot overrule a decline, someone who LEFT the Circle within the same 12 hours cannot be re-added by anyone including the owner, and only the canonical owner may re-add an owner-removed user. A person already at the ten-Circle limit cannot be added and is not named in the refusal. Any pending invitation for an added person is marked accepted. Adding grants no location, SMS, or trusted authorization; the response's `invites` array is retained and always empty |
