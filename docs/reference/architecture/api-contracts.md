...
| POST   | `/api/one/agent-chat/drive-context/preview`           | Vault Owner diagnostic read of the current selected Drive context envelope. Requires exactly `{}`; the server issues a fresh turn ID. Returns metadata and opaque per-envelope refs only, with `private, no-store`; no chat/model invocation or content read. Feature-off is `403`; incomplete authority or excessive selection is a typed `409`. [Envelope v1 contract](../../../consent-protocol/docs/reference/drive-context-envelope.md). |
