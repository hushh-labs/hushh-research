...
| Compute backend seam | interface | GCP only: `gcp` (managed), `user_gcp` (owner project), `null` (inert unconfigured state). Hosted provisioning remains gated | `consent-protocol/hushh_mcp/services/compute_backend.py` |
