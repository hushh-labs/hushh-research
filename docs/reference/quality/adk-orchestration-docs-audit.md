...
**Review basis:** integration checkout based on `11d293dbf7d514c59afa26f8e2a979c5b8539554`, with staged source changes inspected on 2026-09-23. This is a source snapshot, not a deployed release revision. Recheck these findings against the final integration commit before promotion; source inspection does not establish per-environment rollout or cleanup.
...
| Recursive documentation model | Stale split recommendations | [Knowledge model](../operations/documentation-recursive-knowledge-model.md) still proposes splitting the mobile and One Location guides, while `docs/guides/mobile/` and `docs/guides/one-location-uat/` already contain child pages. Reconcile that model in the docs-governance workflow before using it as a planning prompt. |
