...
- transport success means Playwright observed `requestfinished` with an HTTP-success GA response for the exact payload; response headers alone do not pass, and any request ending in `requestfailed` remains a failure
