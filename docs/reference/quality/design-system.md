...
7. Liquid Glass is the production material for filled interactive surfaces (founder decision, 2026-09-20). It is one CSS utility, `morphy-liquid` in `hushh-webapp/app/globals.css` (with `morphy-liquid-neutral` for neutral fills), driven only by the theme accent and the light/dark tokens, so Molten Gold and every future accent inherit it. The `Button` `default`, `destructive`, `secondary` and `outline` variants and the Morphy `fill` variants wear it; circular controls inherit through their radius; a container may opt in with `morphy-liquid-neutral` when it is an interactive or elevated surface, never as decoration on plain content. The material sets `background-image` and `box-shadow` only, so the variant keeps its fill and hover colours. It is static by contract (no `backdrop-filter`, animation, `will-change` or transition of its own; `hushh-webapp/__tests__/ui/liquid-material.contract.test.ts`), which is what keeps it inside the render-performance charter. Arbitrary decorative gradients on chrome remain out.
