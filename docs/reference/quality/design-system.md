...
7. Liquid Glass is the production material for filled interactive surfaces (founder decision, 2026-09-20). It is one CSS utility, `morphy-liquid` in `hushh-webapp/app/globals.css` (with `morphy-liquid-neutral` for neutral fills), driven only by the theme accent and the light/dark tokens, so Molten Gold and every future accent inherit it. The `Button` `default`, `destructive`, `secondary` and `outline` variants and the Morphy `fill` variants wear it. Circular controls wear it too (founder decision, 2026-09-20): the bottom-chrome dock and nav pill carry the neutral material inside their own `!important` rule in `globals.css` (the class cannot be layered on top of that rule) plus the accent halo while a voice session is live; `ShellActionSurface` carries it through its `material` variant (`neutral` for the translucent track, `filled` when the caller paints the accent, `none` when content covers the whole circle, such as an avatar); the filled icon circles in Profile, Settings, the theme toggle and the sheet/drawer close buttons carry `morphy-liquid-neutral`. A control that keeps its own outer shadow passes it through `--liquid-neutral-drop` rather than a second shadow utility. Chips, cards, segmented tabs, sheets, avatars and display-only circles stay off it; a container may opt in with `morphy-liquid-neutral` when it is an interactive or elevated surface, never as decoration on plain content. A gradient fill never shares an element with the material (both paint through `background-image`); the gradient's start colour becomes the fill and `--liquid-base` points at it. The material sets `background-image` and `box-shadow` only, so the variant keeps its fill and hover colours. It is static by contract (no `backdrop-filter`, animation, `will-change` or transition of its own; `hushh-webapp/__tests__/ui/liquid-material.contract.test.ts`), which is what keeps it inside the render-performance charter. Arbitrary decorative gradients on chrome remain out.
...
8. **Apple design grammar** (adopted principles; enforced by shared primitive tests and `verify:design-system` where applicable):
   ...
   - Field geometry: direct-entry controls use `--app-input-radius`, which resolves to the capsule stop `--app-radius-pill`. `Input`, `InputGroup`, `Textarea`, `SelectTrigger`, `CommandInput`, and combobox field shells use it; compound controls keep only their outer shell rounded. Use `--app-form-field-gap` (label/control), `--app-form-related-gap` (label/related action), and `--app-form-section-gap` (primary/secondary sections) instead of local spacing guesses. Credential fallback groups keep the “Can’t get in?” descriptor on one centered line and all available quiet actions on the next centered row inside a capped `max-w-[21rem]` measure. Separate multiple actions with middle-dot separators; never add a second primary CTA, pill, column, or vertical divider. If only one action is available, keep it as the single centered element without a separator. Use `--app-form-section-gap` for the larger pause after the primary unlock action and `--app-form-related-gap` to keep the descriptor close to its action row. Visible link text stays close to the descriptor while the transparent control retains a 44px hit area; do not add pill padding, `self-start`, or a left-anchored row. Recovery, alternate unlock, and Sign out share the same quiet escape row when present.
