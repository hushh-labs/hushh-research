...
- The capability-graph evolution contract now names the frozen pod and ADK ancestor commits. The default CI generator uses refreshed main for the semantic diff while retaining compatible workflow revisions from both merged branches; its mirrors and location workflow-card catalog are regenerated from that owner.
