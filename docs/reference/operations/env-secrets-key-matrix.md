...
| `ONE_LOCATION_NEARBY_PRESENCE_MODE` | `consent-protocol/api/routes/one/location.py` | Y | N | N | env | N | env | N | governed rollout gate: non-production defaults to `uat_simulation`; production requires exact `event_pilot`, otherwise disabled |
