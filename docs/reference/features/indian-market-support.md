...
```
Voice utterance  →  voice_intent_service (nifty/sensex alias)
                 →  /kai/indian-market/quote|indices|search|portfolio
                 →  IndianMarketService (L1 mem → L2 Postgres → L3 yfinance)
                 →  ZerodhaKiteAdapter / UpstoxAdapter  (only when keys present)

Frontend:  IndianIndexBar  →  IndianQuoteCard  →  ExchangeBadge (NSE/BSE)
```
