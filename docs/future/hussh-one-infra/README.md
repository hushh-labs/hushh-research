...
This package promotes the temporary One private-agent infrastructure notes into a durable future-state home. It is intentionally docs-first:
...
| Per-person cloud pod | `partially_exists` in dev | Dev has the per-owner Cloud Run pod and BYOC control path in source; image installation, direct ingress, and owner acceptance require separate live evidence. See the [current architecture diagram](../../reference/architecture/views/deployment.md#dev-byoc-image-and-private-agent-flow). |
