...
- Verified source/publication checkpoint: `84a886b97eb2e1c99e0e0e0a917a134254b78943` on `origin/feat/consumer-mcp`; the latest source/test correction at that checkpoint is `3c72f66f9`. Later documentation-only receipts may advance the branch head; the exact handoff head is always taken from Git. The branch contains the typed owner-pod task, canonical PKM memory, Puppy-bound lifecycle, setup-status, connection, integration, calendar, people, Gmail, email-handoff and finance capability slices described by the dated receipts below. No deployment or hosted migration has been performed; the installed dev evidence remains separate from this branch.
