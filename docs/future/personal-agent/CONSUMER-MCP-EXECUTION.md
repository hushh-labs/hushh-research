...
- `get_hussh_connection` and the read-only `get_hussh_setup_status` now dispatch through the existing MCP server for owner/resource-bound OAuth only. The first returns secure setup/review links; the second reads the existing resumable setup job with sanitized stages and never starts provisioning. Neither grants permission nor claims memory execution. Registered developer credentials keep their existing catalog.
