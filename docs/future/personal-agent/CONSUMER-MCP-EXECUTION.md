...
- Latest source slice: `4b4dcdb4080119bb2e99efad092153e2f31d954e` (12 September 2026; published on `origin/feat/consumer-mcp`; implementation corrections `1fb07a398`, `0e335d3c6`, `2e1a704df`, `1af83a849`, `dfc9bfb88` and `4087829e0`, catalog correction `525fbcd5c`, generated projection `00b8aeac7`, canonical encryption correction `2d73598b1`, with evidence receipts recorded in `5a4ec07a0`). The branch adds durable owner-pod task start/status/result/cancellation through the existing sealed commit log, alongside the earlier email workflow and bounded finance capabilities. Task snapshots are owner/client bound, idempotent and fail closed on replacement or cancellation; uncertain work is never replayed. Durable start/status/cancel now revalidate the Agent One grant and connection generation after the pod call, so revocation or reconnect wins races with late lifecycle results. Lifecycle projections accept only the declared Puppy provider lane, require a bound device for Puppy, and match a new Puppy task to the requested device. Puppy tasks receive the same hub-minted short-lived device grant as synchronous turns, carried only in memory. The pod task route refuses caller-supplied credentials outside the Puppy lane and refuses bare Puppy requests. Consumer memory now includes confirmation-gated deletion with encrypted durable tombstones that prevent replay or regeneration, and the canonical PKM rebuild check proves those tombstones survive sealed-log recovery. New consumer-PKM writes use the canonical `EncryptedPayload` AES-256-GCM format, while legacy blobs without an encoding marker remain readable. A Puppy-targeted consumer task now also requires the pod result to report `provider=puppy`; missing or shared/cloud provider labels are refused before the result reaches the external assistant. Memory response authority is fenced at both gateway and pod route: operation, execution target, provider and deployment identity are derived from trusted boundaries, while only a typed nested result is accepted from the executor. The pod route review ledger explicitly includes the three hub-machine-only task endpoints without widening the owner-direct app surface. The shared pod runtime now logs provider-memory skip decisions without payloads and walls the fingerprinting capability-health route while retaining liveness-only health access. Consumer tool visibility and capability discovery now share one canonical name catalog, and the generated topology projection records its handler hash. Focused checks, Ruff, topology validation, package docs/config checks, packed-runtime verification and whitespace checks pass locally. No deployment or hosted migration has been performed. The previously installed dev evidence remains `d11a5f62d`; this branch has not been deployed.
