...
- Latest implementation candidate: `c6ee56b00` (12 September 2026); the branch head includes the branch-only governance, security-scan, memory-binding, pod-door validation, custody-enrollment race, private pod/Puppy recovery, dev-parity compatibility and Claude-lane upgrade/reviewer corrections. The installed dev candidate remains `d11a5f62d`; no later branch commit has been deployed.
...
- After the healthy `d11a5f62d` deployment, branch-only commits `cf0388cd1`, `4fd85a899`, `e1bc743ae` and `c6ee56b00` were pushed without another deployment. The next CI run is [`34662652008`](https://github.com/hushh-labs/hushh-research/actions/runs/34662652008); its result applies to `c6ee56b00`, while the installed Cloud Run revisions and rollback targets above remain unchanged.
