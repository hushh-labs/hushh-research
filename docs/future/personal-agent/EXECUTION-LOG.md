...
| Mail-event trigger (per-user inbox) | `UserGcpBackend` bootstrap plan gains the BYOC mail-trigger (Pub/Sub topic + `gmail-api-push` publisher binding + pod pull-`subscriber` + daily `watch`-renewal), a **metadata-only doorbell** — the pod opens the mail, Hushh out of the path; Anypoint spec §5 row + §7d + Snippet 5 (Mule listener flow) | (this branch) |
