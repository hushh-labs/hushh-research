...
| **M6 ◐ owner pilot live** | BYOC — pod in the user's own GCP | Provision into the **user's own GCP project** via the existing bootstrap path; same slim image + `ComputeBackend` seam (align with HusshOne Xtreme Compute Burst later) | idempotent teardown; orphaned-instance reconcile; key never persisted; generalized federation and lifecycle evidence | ✅ `UserGcpBackend` renders the pod + bootstrap plan and a real owner dev pod now serves from the verified owner project; ❌ generalized WIF/device-agent flow and real Puppy inference remain evidence gaps | **owner pilot live**; general rollout remains external-gated |
